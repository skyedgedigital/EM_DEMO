    ...
    const fn = async () => {
        try {
         ...
         const parsedData=responseData.map(item => ({
          ...
          ? parsedData
          : parsedData.filter(item => item.employee.department._id === department);
          
          console.log("bbbbbbbbbb",filteredData)


            console.log("aagya response", parsedData)
            const CheckMultipleEntry = filteredData.filter((obj,index)=>
              filteredData.findIndex((item)=> item.employee.UAN === obj.employee.UAN) === index
            )

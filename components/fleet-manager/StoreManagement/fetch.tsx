...
const ViewTool = () => {
  ...
  const handleDelete = async (toolId: any) => {
    const resp = await storeManagementAction.DELETE.deleteStoreManagement(toolId)
    if(resp.success){
  ...
  return (
    <>
      ...
      {
        result && <div className="mt-2 flex p-2 text-3xl items-center justify-center" >
          Tools assigned
        </div>
      }
      {result && (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {result.map((ele) => (
                  <tr key={ele._id} className="hover:bg-gray-100">
                    <td className="px-6 py-4 whitespace-nowrap">{ele.tool.toolName}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {
                        ele.returned ? <span className="text-green-500" >
                          Returned

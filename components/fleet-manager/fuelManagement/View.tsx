...
console.log("yeto resp1data hai",resp1Data)
    ...
    const combinedArray = resp1Data.concat(respData);
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              ...
              {result?.map((ele) => (
                <tr key={ele._id} className="hover:bg-gray-100">
                  ...
                  {ele.entry ? <span>-</span> : <span>{`${ele.duration} Hours`}</span>}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {ele.entry ? (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <button className="text-red-500" onClick={async() => {
                        const resp = await fuelManagementAction.DELETE.deleteFuelManagement(ele._id)
                        if(resp.success){

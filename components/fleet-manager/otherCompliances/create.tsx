...
const Create = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const resp = await complianceAction.CREATE.createCompliance(JSON.stringify(dataObj))
    if(resp.success){
  ...
  return (
    <>
      ...
      >
        ...
        {
            (selectedCompliance === "OTHER") && <span className="text-sm text-blue-500" >
                If OTHER is selected , please enter the description below
            </span>
        }
        {
            ( selectedCompliance === "OTHER" ) && <div className="mb-4">
            <label
              htmlFor="input"
              className="block text-sm font-medium text-gray-700"
            >

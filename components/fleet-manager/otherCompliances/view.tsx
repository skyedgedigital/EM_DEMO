...
const View = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const resp = await complianceAction.FETCH.fetchCompliance(JSON.stringify(searchObj));
    if(resp.success){
      setResults(JSON.parse(resp.data))
      toast.success("Compliances Fetched")
    }
    else{
      toast.error("An Error Occurred");
    }
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {result.map((ele) => (
                  <tr key={ele._id} className="hover:bg-gray-100">
                    <td className="px-6 py-4 whitespace-nowrap">{ele.compliance}</td>

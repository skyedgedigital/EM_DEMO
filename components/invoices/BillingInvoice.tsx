...

{
  ...
  useEffect(() => {
    const fetchLastTwoInvoiceNumbers = async () => {
      const { data, message, error, status, success } =
        await billingInvoiceActions.GET.getLastTwoBillInvoiceNumbers();
  ...
  useEffect(() => {
    const fn = async () => {
      ...
      const updatedItems = await Promise.all(
        items.map(async (element) => {
          ...
          const hsnNumber =
            await billingItemActions.FETCH.fetchHsnNoByBillingItemId(itemId);
    ...
    const summarySheetInfo = async () => {
      const resp = await billingInvoiceActions.GET.getBillingSummaryPdfData(
        submittedBillData
      );
      if (resp.success) {
  ...
  const generatePDF = async (printOrDownload: string) => {
    const originalElementId = `Billing-invoice-${invoiceNumber}`;
    ...
    pdf.html(element, {
      callback: async () => {
        ...
        const fileName = `Billing-invoice-${invoiceNumber}.pdf`;
        ...
        const savedInvoiceResponse =
          await billingInvoiceActions.CREATE.createBillInvoice(
            submittedBillData,
            invoiceNumber
          );
        ...
        const workOrderUpdateResponse =
          await billingWorkOrderActions.UPDATE.updateBillingWorkOrderBalance(
  ...
  const generateSummaryPDF = async (printOrDownload: string) => {
    const originalElementId = `Billing-invoice-${invoiceNumber}-summary`;
    ...
    pdf.html(element, {
      callback: async () => {
        const pdfDataUrl = pdf.output('dataurlstring');
        const fileName = `Billing-invoice-${invoiceNumber}-summary.pdf`;
        if (printOrDownload === 'download') pdf.save(fileName);
        ...
        const storageRef = ref(storage, `Billing-invoices/${fileName}`);
  ...
  const handleAutoGenerateInvoice = async () => {
    try {
      ...
      const resp = await billingInvoiceActions.GET.getLatestBillInvoiceNumber();
      if (resp.success) {
  ...
  return (
    <main className=' w-full flex flex-col gap-1 p-4 pt-20'>
      ...
      <div id='bill-invoice' className=' flex items-center justify-center '>
        <div
          className=' border-[1px] border-gray-700  tracking-wider w-full  text-[0.75rem] font-semibold'
          id={`Billing-invoice-${invoiceNumber}`}
      ...
      <div className='flex items-center justify-center '>
        <div
          id={`Billing-invoice-${invoiceNumber}-summary`}

...
const chalanSchema = z.object({
    ...
    file: z.any().refine((file) => file?.length >= 1, 'File is required.').refine(
    ...
    items: z.array(
      z.object({
        ...
        hours: zodInputStringPipe(z.number().positive('Value must be greater than 0')), // Allow optional hours
...
const EditChalan = () => {
  ...
  const onSubmit: SubmitHandler<FormFields>= async (receivedChalan: FormFields) => {
    ...
    try {
        ...
        const res=await chalanAction.UPDATE.updateChalan(selectedChalan._id,updatedData);
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data, success, error } = await chalanAction.FETCH.getAllChalans();
      ...
      if (success) {
        ...
        toast.error(error || 'can not fetch chalan numbers!');
      }
    };
    fetch();
  }, []);
  useEffect(() => {
    const fetch = async () => {
      ...
      if (res.success) {
        const departments = res.data.map((department) => department.departmentName);
  ...
  useEffect(() => {
    const fetchEngineersData = async () => {
      const res = await engineerAction.FETCH.fetchEngineerByDepartmentName(prefilledDepartment.department);
      
      if (res.success)
      { 
      const engineers=res.data?res.data.map(engineer=>engineer.name):[];
        setFetchedEngineerNames(engineers);
       
  
      }
      if (!res.success) {
        return toast.error(res.message || 'Unable to fetch department details!');
      }
    };
  ...
  useEffect(() => {
    ...
    const setItemsValue = () => {
      ...
      if(selectedChalan?.items?.length>0) { const chalanItems=selectedChalan?.items?.map((item)=>{
  ...
  useEffect(() => {
    const fetchChalanData = async () => {
      const res = await chalanAction.FETCH.getChalanByChalanNumber(chalanNumber);
      if (res.success)
  ...
  useEffect(() => {
    const fetchItemsData = async () => {
      const res = await itemAction.FETCH.fetchAllItemOfWorkOrder(prefilledWorkOrder.workOrder);
...
<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2 px-2 mr-2 border-2 border-black rounded-md bg-slate-200">
<h1 className=' bg-primary-color-extreme/10 text-center py-1 text-base   relative  font-semibold '>
 ...
 {     field.onChange(e);
    setChalanNumber(e)}
        ...
        <FormControl>
        <SelectTrigger>
        {field.value ? <SelectValue placeholder="" /> : "Select Chalan No."}
                    </SelectTrigger>   
 ...
 {     field.onChange(e);
    setPrefilledDepartment({
      ...
      isPrefilled:false
        ...
        <FormControl>
        <SelectTrigger>
        {field.value ? <SelectValue placeholder="" /> : "Select a Department"}
                    </SelectTrigger>
                         </FormControl>
 ...
 {     field.onChange(e);
  setPrefilledWorkOrder({
    ...
    isPrefilled:false
        ...
        {field.value ? <SelectValue placeholder="" /> : "Select Work Order Number"}
                    </SelectTrigger> 
...
<FormField
    ...
    render={({ field }) => (
      <FormItem>
        ...
        {field.value ? <SelectValue placeholder="" /> : "Select an Engineer"}
                    </SelectTrigger> 
...
<FormField
    ...
    render={({ field }) => (
      <FormItem >
        ...
        <FormControl>
          <Input placeholder="Enter location" {...field} className=' bg-white '/>
        </FormControl>
...
<FormField
    ...
    render={({ field : { value, onChange, ...fieldProps }}) => (
      <FormItem>
        <FormLabel>Work Description</FormLabel>
        <FormControl>
          <Input type='file' {...fieldProps} className=' bg-white '
          onChange={(event) =>
  ...
  <FormField
    ...
    render={({ field }) => (
      <FormItem>
        ...
        {field.value ? <SelectValue placeholder="" /> : "Select Status"}
                    </SelectTrigger> 
...
<FormField
  ...
  <h2 className='text-blue-800 font-bold'>Fill Items</h2>
        {form.formState.errors.items && (
                  <p className='text-red-500 italic text-xs'>{form.formState.errors.items.message}</p>
                )}
  {fields.map((field, index) => (
   
               <div className='w-full flex flex-col gap-2' key={field.id}>
<div className='flex flex-row  items-center gap-2'> <h2 className='text-blue-800 font-semibold'>{`Item - ${index+1}`}</h2>
          {index >= 0 && (
                      ...
                      )}</div><div className='w-full  gap-2 flex flex-col md:flex-row '>              
<FormField
    control={form.control}
...
{selectedWorkOrderObjectItemOptionsArray.map((option,index)=>
                  <SelectItem value={option._id.toString()} key={option._id.toString()}>{option.itemName.toString()}</SelectItem>
...
<FormField
    ...
    render={({ field }) => (
      <FormItem>
        ...
        <FormControl>
        <SelectTrigger>
        {field.value ? <SelectValue placeholder="" /> : "Select Vehicle No."}
                    </SelectTrigger>   
...
{     e.preventDefault();             append({ item: '', vehicleNumber: '', unit: 'hours', hours: '', startTime: '', endTime:'' })
}                }>Add Item</Button>
  <Button type="submit" className=' bg-green-500 w-40 '>Submit</Button>
  </div>
</form>
</Form>
  );

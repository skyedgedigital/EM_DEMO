...
type TrainingExamFormData = z.infer<typeof trainingExamSchema>;
...
const CreateTrainingExam = ({
  ...
  const [trainings, setAllTrainings] = useState<
    IFetchAllTrainingSelectedDetailsResponse[]
  >([]);
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<TrainingExamFormData>({
    resolver: zodResolver(trainingExamSchema), // Integrate Zod resolver
    defaultValues: {
      allowedCandidates: [],
      questions: [],
  ...
  const fillTitleANDResponsibility = (trainingId: string) => {
  ...
  useEffect(() => {
    if (presetAlreadyProvided) {
      ...
      if (trainings.length > 0) {
        fillTitleANDResponsibility(presetTrainingId);
      }
    }
  }, [presetAlreadyProvided, presetTrainingId, trainings]);
  ...
  const onSubmit = async (submittedFormData: TrainingExamFormData) => {
    try {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-2'>
        <div className='flex flex-col md:flex-row gap-2 items-center'>
          <div className='flex flex-col gap-1 flex-grow p-1'>
            <label
              htmlFor='trainingId'
              className='text-nowrap flex justify-start items-center gap-1'
            >
              <>Select Training:</>
              {loadingStates.fetchingTrainings && (
            ...
            <select
              ...
              onChange={(e) => fillTitleANDResponsibility(e.target.value)}
              disabled={presetAlreadyProvided}
              className='border-[1px] border-gray-400 text-gray-600 bg-gray-50 p-1 rounded disabled:cursor-not-allowed'
            >
          ...
          <div className='flex flex-col gap-1 flex-grow p-1 hover:cursor-not-allowed'>
            <label
              htmlFor='title'
            ...
            <input
              ...
              disabled
              {...register('title')}
              className='border-[1px] border-gray-400 text-gray-500 bg-gray-50 p-1 rounded hover:cursor-not-allowed'
            />
          </div>

          <div className='w-1/2 flex flex-col gap-1 flex-grow p-1 hover:cursor-not-allowed'>
            <label
        ...
        <button
          ...
          type='submit'
          className='bg-green-700 disabled:bg-green-500 flex justify-center items-center gap-2 text-white rounded w-fit px-4 py-1 mx-auto my-4'
        >
          <>Create Training</>
...
export default CreateTrainingExam;

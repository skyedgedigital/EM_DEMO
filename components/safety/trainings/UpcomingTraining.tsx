'use client';
import { IUpcomingTrainingsResponse } from '@/lib/actions/safety/training/fetch';
...
const UpcomingTraining = () => {
  const [upcomingTrainings, setUpcomingTrainings] =
    useState<IUpcomingTrainingsResponse[]>(null);
  const [loadingStates, setLoadingStates] = useState({
    loadingUpcomingTrainings: true,
  });
  const fetchUpComingTraining = async () => {
    try {
      setLoadingStates((prev) => ({ ...prev, loadingUpcomingTrainings: true }));
      const { data, status, success, message, error } =
        await trainingActions.FETCH.fetchUpcomingTrainings();
      ...
      setLoadingStates((prev) => ({
        ...prev,
        loadingUpcomingTrainings: false,
      }));
    }
  };
  useEffect(() => {
      ...
      {loadingStates.loadingUpcomingTrainings ? (
        ...
        <div className='flex flex-col justify-center items-center gap-2'>
          {upcomingTrainings?.map((training, index) => (
            <Link
              target='_blank'
              href={`/safety/trainings/training-details?trainingId=${training._id}`}
              key={training.title}

...
const fetchEmployeesWithWorkorderHr = async (): Promise<ApiResponse<any>> => {
  ...
  try {
    ...
    return {
      ...
      data: JSON.stringify(sortedDocs),
      error: null,
    };
  } catch (err) {
    console.error(err);
...
export interface IEmployeesBySelectedHRWorkOrderIDResponse
  extends Partial<IEmployeeData> {
  _id: string;
}
const fetchEmployeesBySelectedHRWorkOrderID = async (
  HrWorkOrderID: string,
  selectFields: (keyof IEmployeeData)[] = []
): Promise<ApiResponse<IEmployeesBySelectedHRWorkOrderIDResponse[]>> => {
  const dbConnection = await handleDBConnection();
  if (!dbConnection.success) return dbConnection;
  try {
    // console.log("ID TO SEARCH FOR", HrWorkOrderID);
    let docs = [];
    if (!HrWorkOrderID) {
      // console.log("SEARCHING ALL");
      docs = await EmployeeData.find();
    } else {
      // console.log("SEARCHING SOME");
      docs = await EmployeeData.find({
        workOrderHr: {
          $elemMatch: {
            workOrderHr: new mongoose.Types.ObjectId(HrWorkOrderID),
          },
        },
      })
        .sort({ date: -1 })
        .populate('designation')
        .select(selectFields.length > 0 ? selectFields.join(' ') : '');
    }
    // console.log("FOUND EMPLOYEE LENGHT", docs.length)
    const sortedDocs = docs.sort((a, b) => a.name.localeCompare(b.name));
    return {
      success: true,
      status: 200,
      message: 'Fetched all employees with workorder',
      data: JSON.parse(JSON.stringify(sortedDocs)),
      error: null,
    };
  } catch (err) {
    console.error(err);
...
const fetchAllEmployeesSelectedFieldInfos = async (
  selectFields: (keyof IEmployeeData)[] = []
): Promise<ApiResponse<any>> => {
  const dbConnection = await handleDBConnection();
  if (!dbConnection.success) return dbConnection;
  try {
    ...
    const docs = await EmployeeData.find({}).select(selectFields.join(' '));
    // .populate('designation');
    const sortedResp = docs.sort((a, b) => a.name.localeCompare(b.name));

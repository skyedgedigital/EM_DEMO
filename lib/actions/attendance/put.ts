...
const putAttendance = async(dataString:string,filter:string) => {
    try{
        ...
        if(doc){
            ...
            attendanceArray.forEach( (ele:any) => {
                ...
                const existing = currentAttendanceArray.find( ele => ele.day === givenDay )
                if(existing){
            ...
            const updatedData = { $set: { days: currentAttendanceArray , workOrderHr: workOrder } };
            const resp = await Attendance.findOneAndUpdate(filterData,updatedData,{new:true})
            console.log(resp)
            ...
            if (resp) {
                ...
                if (employee) {
                    const existingPeriod = employee.workOrderHr.find(entry => 
                    ...
                    if (existingPeriod) {
                        ...
                        await EmployeeData.updateOne(
                            { _id: emp },
                            { $addToSet: { workOrderHr: { period, workOrderHr: workOrder, workOrderAtten: presentDaysCount } } }
                        ); 

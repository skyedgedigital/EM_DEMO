...
const getSummaryPdfData = async (invoiceId: string) => {
    try {
      await connectToDB();
      ...
      const map = new Map<string, { from: Date; to: Date , locations: Set<string> }>();
      const addValueToMap = (key: string, date: Date,location:string) => {
        const dateObj = new Date(date);
  
        if (!map.has(key)) {
          map.set(key, { from: dateObj, to: dateObj, locations:new Set([location]) });
        } else {

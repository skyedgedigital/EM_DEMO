...
const mergeChalans = async(chalanNumbers:string[]) => {
    try{
        console.log("hi")
        const existresp=await checkIfExisting(chalanNumbers);
        if(existresp.success && existresp.status==200)
       { const chalans = await Promise.all(chalanNumbers.map(async (chalanNumber) => {
        ...
        await Promise.all(chalans.map(async (chalan) => {
